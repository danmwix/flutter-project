...
class _LoginScreenState extends State<LoginScreen> {
  ...
  final _resetAdmissionNumberController = TextEditingController();
  ...
  void dispose() {
    ...
    _resetAdmissionNumberController.dispose();
    super.dispose();
  }
        ...
        if (userQuery.docs.isNotEmpty) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Invalid admission number or password'), backgroundColor: Color(0xFFF44336)),
            );
          }
        }
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Form(
          key: formKey,
          child: TextFormField(
            controller: _resetAdmissionNumberController,
            ...
            onPressed: () async {
              if (formKey.currentState!.validate()) {
                try {
                  QuerySnapshot userQuery = await FirebaseFirestore.instance
                      .collection('users')
                      .where('admissionNumber', isEqualTo: _resetAdmissionNumberController.text.trim())
                      .limit(1)
                      .get();
                  ...
                  if (userQuery.docs.isNotEmpty) {
                    ...
                    if (mounted) {
                      Navigator.pop(context);
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('No account found with this admission number'), backgroundColor: Color(0xFFF44336)),
                      );
                    }
                  }
                } catch (e) {
